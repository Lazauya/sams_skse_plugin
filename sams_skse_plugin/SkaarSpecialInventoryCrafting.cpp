#include "samsSpecialInventoryCrafting.h"
...
public:
	...
	ExtraContainerChanges::EntryDataList *samsEntryDataListWithExistingItemCounts(ExtraContainerChanges::EntryDataList *givenItemsList) {
	...
	ExtraContainerChanges::EntryDataList *samsAddRemainingItems(ExtraContainerChanges::EntryDataList *currentEntries, FormType type) {
		...
		while (iteratorPosition != iteratorEnd) {
			...
			if (entryData && entryData->countDelta > 0) {
				if (entryData->type->GetFormType() == type || type == kFormType_None) {
					...
					bool alreadyThere = samsSpecialInventoryCrafting::samsEntryDataListContainsEntryData(currentEntries, entryData);
					if (!alreadyThere) {
...
namespace samsSpecialInventoryCrafting {
	...
	ExtraContainerInfo samsItemInfoForObjectReference(TESObjectREFR* pContainerRef);
	ExtraContainerChanges::EntryDataList *samsAddRemainingItems(ExtraContainerChanges::EntryDataList *currentItems, TESObjectREFR *pContainerRef, FormType type);
	...
	ExtraContainerChanges::EntryDataList *samsGetAllItems(TESObjectREFR* container, FormType type);
	...
	void samsAddItemsFromContainerToContainer(StaticFunctionTag *base, TESObjectREFR* pSourceContainerRef, TESObjectREFR* pDestContainerRef, UInt32 typeID) {
		...
		_MESSAGE("samsAddItemsFromContainerToContainer() called");
		...
		ExtraContainerChanges::EntryDataList *sourceContainerItems = samsGetAllItems(pSourceContainerRef, type);

		// Get the list of things to add to the destination container
		ExtraContainerInfo destContainerInfo = samsItemInfoForObjectReference(pDestContainerRef);
		ExtraContainerChanges::EntryDataList *newEntryDataList = destContainerInfo.samsEntryDataListWithExistingItemCounts(sourceContainerItems);
		
		// Put into the list those items that were already on the destination but not in the source so they aren't lost
		newEntryDataList = samsAddRemainingItems(newEntryDataList, pDestContainerRef, type);
		...
		_MESSAGE("samsAddItemsFromContainerToContainer() finished");
	}
	...
	void samsRemoveItemsInContainerFromContainer(StaticFunctionTag *base, TESObjectREFR* pInContainerRef, TESObjectREFR* pFromContainerRef, UInt32 typeID) {
		...
		ExtraContainerChanges::EntryDataList *inContainerItems = samsGetAllItems(pInContainerRef, type);
		if (!inContainerItems || !(inContainerItems->Count() > 0)) {
			return;
		}

		ExtraContainerChanges::EntryDataList *fromContainerItems = samsGetAllItems(pFromContainerRef, type);
	...
	ExtraContainerChanges::EntryDataList *samsAddRemainingItems(ExtraContainerChanges::EntryDataList *currentItems, TESObjectREFR *pContainerRef, FormType type) {
		ExtraContainerInfo containerInfo = samsItemInfoForObjectReference(pContainerRef);
		return containerInfo.samsAddRemainingItems(currentItems, type);
	}
	...
	bool RegisterFuncs(VMClassRegistry* registry) {
		registry->RegisterFunction(
			new NativeFunction3 <StaticFunctionTag, void, TESObjectREFR*, TESObjectREFR*, UInt32>("samsAddItemsFromContainerToContainer", "samsSpecialInventoryCrafting", samsSpecialInventoryCrafting::samsAddItemsFromContainerToContainer, registry));

		registry->RegisterFunction(
			new NativeFunction3 <StaticFunctionTag, void, TESObjectREFR*, TESObjectREFR*, UInt32>("samsRemoveItemsInContainerFromContainer", "samsSpecialInventoryCrafting", samsSpecialInventoryCrafting::samsRemoveItemsInContainerFromContainer, registry));
